...
def place_buy(market, eur_amount, entry_price, order_type=None, *, is_dca: bool = False):
    ...
    if not is_dca:
        ...
        if cooldown_until and time.time() < cooldown_until:
            ...
            log(
                f"🛡️ Saldo Guard cooldown actief ({remaining}s resterend) — entry {market} overgeslagen", level="warning"
            )
    ...
    if total_slots_used >= max_trades and market not in open_trades:
        log(
            f"Max open trades limiet bereikt ({current} active + {pending_orders} pending orders + {reserved} reserved = {total_slots_used}/{max_trades}), geen nieuwe trade voor {market}",
            level="warning",
        )
    ...
    base = market.split("-")[0]
    cluster_positions = [
        t for mk, t in (open_trades or {}).items() if isinstance(mk, str) and mk.startswith(f"{base}-")
    ]
    MAX_CLUSTER_TRADES_PER_BASE = int(CONFIG.get("MAX_CLUSTER_TRADES_PER_BASE", max(1, max_trades // 2)))
    MAX_CLUSTER_EXPOSURE_EUR = float(
        CONFIG.get("MAX_CLUSTER_EXPOSURE_EUR", CONFIG.get("MAX_TOTAL_EXPOSURE_EUR", 300.0))
    )
    if market not in open_trades and len(cluster_positions) >= MAX_CLUSTER_TRADES_PER_BASE:
        log(
            f"Clusterlimiet bereikt voor {base}: {len(cluster_positions)}/{MAX_CLUSTER_TRADES_PER_BASE}, skip {market}",
            level="warning",
        )
        return {"error": "cluster_limit_reached"}
    try:
        cluster_exposure = sum(
            float(t.get("amount", 0.0) or 0.0) * float(t.get("buy_price", 0.0) or 0.0) for t in cluster_positions
        )
    except Exception:
        cluster_exposure = 0.0
    if market not in open_trades and (cluster_exposure + eur_amount) > MAX_CLUSTER_EXPOSURE_EUR:
        log(
            f"Cluster exposure te hoog voor {base}: {cluster_exposure + eur_amount:.2f}>{MAX_CLUSTER_EXPOSURE_EUR:.2f}, skip {market}",
            level="warning",
        )
    ...
    try:
        ...
        if max_total_exp < 9000:
            ...
            if (cur_exp + eur_amount) > max_total_exp:
                log(
                    f"Total exposure limiet bereikt: {cur_exp:.2f}+{eur_amount:.2f}={cur_exp + eur_amount:.2f} > {max_total_exp:.2f} EUR, skip {market}",
                    level="warning",
                )
    ...
    try:
        ...
        if _budget_cfg.get("enabled", False):
            ...
            if (trailing_cur_exp + eur_amount) > trailing_effective_max:
                log(
                    f"Trailing budget limiet: {trailing_cur_exp:.2f}+{eur_amount:.2f}={trailing_cur_exp + eur_amount:.2f} > {trailing_effective_max:.2f} EUR (base {trailing_max_base:.0f} + profit {trailing_profit_bonus:.2f}), skip {market}",
                    level="warning",
                )
        ...
        if _daily_limit < 9000 or _weekly_limit < 9000:
            ...
            if os.path.exists(_tl_path):
                ...
                if _daily_limit < 9000 and _daily_loss >= _daily_limit:
                    log(
                        f"🛑 Daily loss limiet bereikt: {_daily_loss:.2f} >= {_daily_limit:.2f} EUR, skip {market}",
                        level="warning",
                    )
                    return {"error": "daily_loss_limit"}
                if _weekly_limit < 9000 and _weekly_loss >= _weekly_limit:
                    log(
                        f"🛑 Weekly loss limiet bereikt: {_weekly_loss:.2f} >= {_weekly_limit:.2f} EUR, skip {market}",
                        level="warning",
                    )
    ...
    try:
        ...
        if eur_bal < min_required:
            log(
                f"EUR balans te laag ({eur_bal:.2f} < {min_required:.2f}), {'DCA' if is_dca else 'nieuwe trade'} voor {market} overgeslagen",
                level="warning",
            )
    ...
    if S.AUTO_USE_FULL_BALANCE:
        try:
            ...
            if eur_bal and eur_bal > 0:
                ...
                log(
                    f"AUTO_USE_FULL_BALANCE active: available EUR={eur_bal:.2f}, using {use_amount:.2f} EUR for this buy"
                )
    ...
    if use_limit and book:
        ...
        if float(q_px) > 0:
            ...
            if effective_quote < _min_quote and effective_quote > 0:
                ...
                q_amt += one_tick
        params = {"amount": float(q_amt), "price": float(q_px)}
        ...
        if (
            ...
            and resp.get("errorCode") in (101, 429)
            and "decimal" in str(resp.get("error", "")).lower()
        ):
            tighter = Decimal(str(amount_base)).quantize(
                Decimal("1." + "0" * max(0, amt_decimals - 1)), rounding=ROUND_DOWN
            )
        ...
        params = {"amountQuote": float(q_eur)}
        OPERATOR_ID = S.OPERATOR_ID
        if OPERATOR_ID:
            params["operatorId"] = OPERATOR_ID
...
def place_sell(market, amount_base, *, skip_dust: bool = False, sell_all: bool = False):
                ...
                if retries >= max_retries:
                    ...
                    closed_entry = {
                        ...
                        "total_invested_eur": round(
                            float(t.get("total_invested_eur", invested_eur) or invested_eur), 4
                        ),
                    ...
                    log(
                        f"[saldo_error] {market}: retry {retries}/{max_retries} — trade blijft open, wacht op sync",
                        level="warning",
                    )
    ...
    if sell_amount < amount_base * 0.95:
        log(
            f"Waarschuwing: verkoophoeveelheid ({sell_amount:.8f}) veel lager dan gevraagd ({amount_base:.8f}) voor {symbol}"
        )
    if sell_all and available > amount_base * 1.001:
        log(
            f"[sell_all] {symbol}: verkoop volledig saldo {available:.8f} (trade amount was {amount_base:.8f})",
            level="info",
        )
    ...
    if sell_all:
        ...
        norm_amount = float(Decimal(str(sell_amount)).quantize(Decimal("1." + "0" * max(0, prec)), rounding=ROUND_DOWN))
    else:
    ...
    if min_size > 0 and norm_amount < min_size:
        log(
            f"⚠️ Sell amount {norm_amount:.8f} < min order size {min_size:.8f} for {market}, trying full available balance {available:.8f}",
            level="warning",
        )
        norm_amount = S.normalize_amount(market, available)
        if norm_amount < min_size:
            log(
                f"❌ Even full balance {norm_amount:.8f} < min order {min_size:.8f} for {market} — dust trade, skipping sell",
                level="warning",
            )
    ...
    def _place_sell_order(amount: float):
        ...
        if S.LIVE_TRADING and not S.PLACE_ORDERS_ENABLED:
            log(
                f"Blocked SELL for {market}: missing BITVAVO_OPERATOR_ID (operatorId). Trade not sent to Bitvavo.",
                level="error",
            )
            return {"error": "operator_id_missing"}
        OPERATOR_ID = S.OPERATOR_ID
        ...
        if isinstance(order_resp, dict) and (
            order_resp.get("errorCode") in (101, 429)
            or "decimal" in str(order_resp.get("error", "")).lower()
            or "minimum" in str(order_resp.get("error", "")).lower()
        ):
    ...
    if chunk_count > 1:
        ...
        if remaining > 0:
            ...
            if _rem_norm >= _rem_min and _rem_norm > 0:
                ...
                log(
                    f"[chunk] Remaining {_rem_norm:.8f} {symbol} below min order {_rem_min:.8f} — unsellable dust",
                    level="warning",
                )
    ...
    if sell_all and isinstance(resp, dict) and not resp.get("error") and not resp.get("errorCode"):
        try:
            ...
            if _post_avail > 0:
                ...
                prec = S.get_amount_precision(market)
                _post_norm = float(
                    Decimal(str(_post_avail)).quantize(Decimal("1." + "0" * max(0, prec)), rounding=ROUND_DOWN)
                )
                _min_eur = 5.0  # Bitvavo minimum order value in EUR
                if _post_norm >= _post_min and _post_norm > 0 and _post_val >= _min_eur:
                    log(
                        f"[sell_sweep] {symbol}: {_post_avail:.8f} remaining after sell (EUR {_post_val:.2f}), selling remainder",
                        level="info",
                    )
                    ...
                    log(
                        f"[sell_sweep] {symbol}: {_post_avail:.8f} remaining (EUR {_post_val:.2f}) below min order (base={_post_min}, min EUR={_min_eur}) — unsellable dust",
                        level="debug",
                    )
...
def _verify_sell_response(sell_response: dict, market: str, expected_amount: float) -> tuple:
    ...
    if sell_response.get("chunked"):
        ...
        if remaining > expected_amount * 0.05:
            log(
                f"⚠️ Chunked sell for {market} left {remaining:.8f} unsold ({remaining / expected_amount * 100:.1f}%)",
                level="warning",
            )
