...
def sync_with_bitvavo():
    ...
    try:
        ...
        if pre_sync_count == 0 and live_count > 0:
            log(
                f"🔴 CRITICAL: open_trades is EMPTY but Bitvavo has {live_count} positions! "
            ...
            for m, entry in live_open.items():
                if m in open_trades:
                    ...
                    if _amount_change_pct > 0.001:  # >0.1% change
                        _need_derive = True
                        _derive_reason = (
                            f"amount changed {old_amount:.6f} → {new_amount:.6f} ({_amount_change_pct * 100:.2f}%)"
                        )
                    ...
                    if not _need_derive:
                        ...
                        if _cur_bp > 0 and new_amount > 0 and _cur_inv > 0:
                            ...
                            if abs(_cur_inv - _expected) / max(_expected, 0.01) > 0.02:
                                _need_derive = True
                                _derive_reason = (
                                    f"invested_eur €{_cur_inv:.2f} diverges >2% from expected €{_expected:.2f}"
                                )

                    if _need_derive:
                        if _override_cb:
                            log(
                                f"[{m}] skip derive_cost_basis (manual override active): {_derive_reason}",
                                level="debug",
                            )
                            _need_derive = False
                    if _need_derive:
                        try:
                            ...
                            if _basis and getattr(_basis, "avg_price", 0) > 0:
                                ...
                                log(
                                    f"⚠️ DERIVE [{m}] failed ({_derive_reason}): no result from order history",
                                    level="warning",
                                )
                                ...
                                if _fb_bp > 0 and new_amount > 0:
                                    ...
                                    if _cur_inv <= 0 or abs(_cur_inv - _fb_expected) / max(_fb_expected, 0.01) > 0.05:
                                        log(
                                            f"⚠️ FALLBACK [{m}]: invested_eur €{_cur_inv:.2f} → €{_fb_expected:.2f}",
                                            level="warning",
                                        )
                    ...
                    try:
                        if entry.get("highest_price") and (
                            local.get("highest_price") is None
                            or entry.get("highest_price") > local.get("highest_price")
                        ):
                    ...
                    local.setdefault(
                        "cost_buffer_pct",
                        float(CONFIG.get("FEE_TAKER", 0.0025)) * 2 + float(CONFIG.get("SLIPPAGE_PCT", 0.001)),
                    )
                    ...
                    try:
                        ...
                        if n_restored:
                            log(
                                f"[entry_metadata] {m}: restored {n_restored} fields from cache (was sync_attach)",
                                level="info",
                            )
                    except Exception:
                        pass
                    ...
                    try:
                        ...
                        base_price = float(
                            local.get("buy_price") or entry.get("buy_price") or entry.get("highest_price") or 0.0
                        )
                    ...
                    if initial_inv <= 0:
                        try:
                            ...
                            if basis and getattr(basis, "avg_price", 0) > 0:
                                ...
                                local["dca_events"] = []
                                log(
                                    f"Sync: Derived initial values for NEW trade {m}: invested=€{basis.invested_eur:.2f}",
                                    level="info",
                                )
                        except Exception as e:
                    ...
                    try:
                        local["highest_price"] = max(
                            float(local.get("highest_price") or 0.0), float(local.get("buy_price") or 0.0)
                        )
                    except Exception as e:
                    ...
                    try:
                        new_local = {
                            ...
                            "cost_buffer_pct": float(CONFIG.get("FEE_TAKER", 0.0025)) * 2
                            + float(CONFIG.get("SLIPPAGE_PCT", 0.001)),
                            "score": 0.0,
                        ...
                        try:
                            ...
                            if n_restored:
                                log(
                                    f"[entry_metadata] {m}: restored {n_restored} fields on new_local (was lost from open_trades)",
                                    level="info",
                                )
                        except Exception:
                            pass
                    ...
                    try:
                        ...
                        if basis and getattr(basis, "avg_price", 0) > 0:
                            ...
                            try:
                                ...
                                if recovered:
                                    ...
                                    _ti_set_initial(
                                        new_local, recovered["invested_eur"], source="sync_archive_recovery"
                                    )
                                    log(
                                        f"Sync: Recovered {m} cost from archive: €{recovered['invested_eur']:.2f} ({recovered['source']})",
                                        level="info",
                                    )
                            except Exception:
                                pass
                        ...
                        try:
                            ...
                            if recovered:
                                ...
                                log(
                                    f"Sync: Recovered {m} cost from archive after derive failure: €{recovered['invested_eur']:.2f} ({recovered['source']})",
                                    level="info",
                                )
                        except Exception:
                            pass
                    ...
                    try:
                        new_local["highest_price"] = max(
                            float(new_local.get("highest_price") or 0.0), float(new_local.get("buy_price") or 0.0)
                        )
                    ...
                    if current_count >= max_trades:
                        log(
                            f"⚠️ SYNC OVER LIMIT: Adding {m} ({current_count + 1}/{max_trades}) — "
                    ...
                    if m not in live_open:
                        ...
                        try:
                            closed_entry = {
                                ...
                                "profit": -t.get("buy_price", 0.0) * t.get("amount", 0.0)
                                if isinstance(t, dict) and t.get("buy_price")
                                else 0.0,
                                "timestamp": time.time(),
                                "reason": "sync_removed",
