...
def cancel_open_buys_by_age() -> None:
    ...
    try:
            ...
            try:
                ...
                if not created_ms:
                    if not getattr(cancel_open_buys_by_age, "_missing_ts_logged", False):
                        state.log(
                            f"Skip cancel_open_buys_by_age: no timestamp for order {o.get('orderId')} ({market})",
                            level="debug",
                        )
        ...
        for order_id, market, age in to_cancel:
            if _cancel_one(market, order_id):
                state.log(
                    f"Canceled open BUY order {order_id} for {market} due to timeout ({int(age)}s >= {timeout}s)",
                    level="warning",
                )
                success += 1
