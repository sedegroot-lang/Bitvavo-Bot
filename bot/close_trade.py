...
_OPERATIONAL_REASONS = {'saldo_error', 'sync_removed', 'manual_close',
    ...
    *,
    ...
    try:
        if cfg.get('BAYESIAN_FUSION_ENABLED', True):
            ...
            _active_sigs = {sn: True for sn in (
                'sma_cross', 'price_above_sma', 'rsi_ok', 'macd_ok', 'ema_ok',
                'bb_breakout', 'stoch_ok', 'trend_1m', 'trend_5m', 'trend_5m_strong',
                'breakout', 'vol_above_avg', 'rsi_momentum')}
    ...
    try:
        if cfg.get('META_LEARNER_ENABLED', True):
            ...
            _sma_cross = (trade and trade.get('sma_short_at_entry') and trade.get('sma_long_at_entry')
        ...
        if _sp is not None:
            ...
            if trade and trade.get('opened_ts'):
                try:
                    ...
                    _hold_h = None
            _sp.publish_sell(
                market, _bp, _sp_price, _profit, _pct,
