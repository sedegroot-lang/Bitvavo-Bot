...
def _signal_strength_impl(m: str) -> Tuple[float, Any, Any, dict]:
    ...
    ml_conf = 0.0
    try:
        from modules.ml import feature_engineering, predict_ensemble, prepare_lstm_sequence

        # Build 7-feature array matching the XGBoost model (rsi, macd, sma_short, sma_long, volume, bb_position, stochastic_k)
        _sma_s = float(
            s_short[-1]
            if s_short is not None and hasattr(s_short, "__getitem__")
            else (s_short if s_short is not None else p1[-1])
        )
        _sma_l = float(
            s_long[-1]
            if s_long is not None and hasattr(s_long, "__getitem__")
            else (s_long if s_long is not None else p1[-1])
        )
        _bb_u = float(
            bb_upper[-1]
            if bb_upper is not None and hasattr(bb_upper, "__getitem__")
            else (bb_upper if bb_upper is not None else p1[-1] * 1.02)
        )
        _bb_l = float(
            bb_lower[-1]
            if bb_lower is not None and hasattr(bb_lower, "__getitem__")
            else (bb_lower if bb_lower is not None else p1[-1] * 0.98)
        )
        ...
        if len(_ml_signal_history) >= 100:
            ...
            if hold_pct >= _ml_veto_alert_threshold:
                log(
                    f"[ML ALERT] High HOLD frequency: {hold_pct * 100:.1f}% ({hold_count}/100) - Model retrain recommended",
                    level="warning",
                )

        log(
            f"[Ensemble] {m}: signal={ml_signal} (conf={ml_conf:.2f}, xgb={result.get('xgb_signal')}, lstm={result.get('lstm_prediction')})",
            level="info",
        )
        if ml_signal == 1:
    ...
    ml_info = {
        ...
        "score_before_ml": score_before_ml,
        "ml_boost": ml_boost,
        "ml_weight": ml_weight,
    ...
    try:
        ...
        try:
            ...
            if hasattr(_shared_state, "regime_engine") and _shared_state.regime_engine:
                regime_hint = str(
                    getattr(_shared_state.regime_engine, "current_regime", "neutral") or "neutral"
                ).lower()
        except Exception:
            pass
        ...
        if _cfg.get("SIGNALS_DEBUG_LOGGING"):
            log(
                f"[entry_conf] {m} conf={ec.confidence:.3f} weakest={ec.weakest_pillar} pillars={ec.pillars}",
                level="debug",
            )
