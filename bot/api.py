...
from bot.helpers import as_bool, as_float, as_int
...
def get_candles(market: str, interval: str = "1m", limit: int = 120, start: Any = None, end: Any = None) -> list:
...
def get_expected_slippage(market: str, amount_eur: float, entry_price: float) -> Optional[float]:
    ...
    if not book or "asks" not in book:
        return None
    try:
        asks = [(float(px), float(sz)) for px, sz, *_ in book.get("asks", []) if px is not None and sz is not None]
        if not asks:
            return None
...
def get_expected_slippage_sell(market: str, amount_base: float, ref_price: float) -> Optional[float]:
    ...
    if not book or "bids" not in book:
        return None
    try:
        bids = [(float(px), float(sz)) for px, sz, *_ in book.get("bids", []) if px is not None and sz is not None]
        if not bids:
            return None
...
def normalize_amount(market: str, amount: float) -> float:
    try:
        ...
        if not step or step <= 0:
            return float(Decimal(str(amount)).quantize(Decimal("1." + "0" * prec), rounding=ROUND_DOWN))
...
def normalize_price(market: str, price: float) -> float:
    try:
        ...
        if not step or step <= 0:
            prec = get_price_precision(market)
            return float(Decimal(str(price)).quantize(Decimal("1." + "0" * prec), rounding=ROUND_DOWN))
