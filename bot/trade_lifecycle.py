...
def save_trades(force: bool = False):
    ...
    log(
        f"Trade analyse: win ratio={win_ratio:.2f}, avg win={avg_win:.2f}, avg loss={avg_loss:.2f}, avg profit={avg_profit:.2f}"
    )
    with trades_lock:
        ...
        if S.REINVEST_ENABLED:
            ...
            if recent_count >= S.REINVEST_MIN_TRADES and recent_profit >= S.REINVEST_MIN_PROFIT:
                ...
                if new_base > S.BASE_AMOUNT_EUR:
                    ...
                    log(
                        f"Reinvest: toegevoegd {add:.2f} EUR aan BASE_AMOUNT_EUR (van {old} -> {CONFIG['BASE_AMOUNT_EUR']}) op basis van winst {recent_profit:.2f} over {recent_count} trades"
                    )
            else:
    ...
    try:
        ...
        for _vm, _vt in list(open_trades.items()):
            ...
            try:
                ...
                if _vdev > 0.50:
                    log(
                        f"⚠️ STARTUP VALIDATION: {_vm} buy_price €{_vbp:.6f} deviates {_vdev * 100:.1f}% from ticker €{_vticker_p:.6f}. Re-deriving.",
                        level="warning",
                    )
                    try:
                        ...
                        if _vbasis and getattr(_vbasis, "avg_price", 0) > 0:
                            ...
                            _validation_fixes += 1
                            log(
                                f"✅ STARTUP FIX {_vm}: buy_price €{_old_bp:.6f} → €{_vt['buy_price']:.6f}",
                                level="warning",
                            )
