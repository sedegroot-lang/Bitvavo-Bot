...
class SyncValidator:
    ...
    def auto_fix_phantom_positions(self, dry_run=True) -> int:
        ...
        for symbol, pos in bot_positions.items():
            ...
            if symbol not in bitvavo_balances:
                fixes.append(
                    {"symbol": symbol, "market": pos["market"], "amount": pos["amount"], "action": "remove_phantom"}
                )
    ...
    def auto_add_missing_positions(self, dry_run=True) -> int:
        ...
        self._log(
            f"Syncing {len(additions)} existing Bitvavo positions to trade_log (bypass MAX_OPEN_TRADES - already bought)"
        )
        ...
        if dry_run:
            ...
            for add in additions:
                self._log(
                    f"  - Add {add['market']}: {add['amount']:.8f} @ €{add['price']:.4f} = €{add['invested']:.2f}"
                )
            return 0
