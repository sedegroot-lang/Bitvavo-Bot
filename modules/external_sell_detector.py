...
def detect_external_sells(bitvavo_client, trade_log_path: Path, markets: list[str]) -> dict[str, dict]:
    ...
    for market in markets:
        ...
        try:
            ...
            for t in sorted(trades_response, key=lambda x: float(x.get('timestamp', 0) or 0) / (1000 if float(x.get('timestamp', 0) or 0) > 10000000000 else 1), reverse=True):
                ...
                if buys_after:
                    ...
                    if last_sell_trade:
                        try:
                            ...
                            order_id = last_sell_trade.get('orderId') or last_sell_trade.get('orderID') or last_sell_trade.get('order_id')
                            base_currency = market.split('-')[0].upper()
                    ...
                    logger.info(f"[{market}] Reset needed: new invested=EUR{new_invested:.2f}, opened={results[market]['new_opened_date']}")
...
def apply_external_sell_resets(trade_log_path: Path, resets: dict[str, dict]) -> int:
    ...
    for market, reset_data in resets.items():
        ...
        if sell_snapshot.get('sell_price') and sell_snapshot.get('sell_amount'):
            try:
                ...
                closed_entry = {
                    ...
                    'sell_date': datetime.fromtimestamp(reset_data.get('last_sell_ts', datetime.now().timestamp())).isoformat(),
        ...
        logger.info(f"[{market}] Trade reset applied: invested=EUR{reset_data['new_invested']:.2f}, opened={reset_data['new_opened_date']}")
        count += 1
