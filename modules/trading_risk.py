...
class RiskManager:
	...
	def refresh(self, *, force: bool = False) -> None:
		...
		with self._lock:
			...
			self.ctx.log(
				"Risk metrics geüpdatet: global_dd={:.2f}EUR, thresholds={}".format(
					current_dd, segment_thresholds
				),
				level="debug",
			)
	...
	def assess_new_trade(
		...
		*,
		...
		try:
			...
			if open_trades and 'BTC-EUR' in open_trades:
				...
				if total > 0 and correlated / total > 0.7:
					reason = f"Portfolio >70% gecorreleerd met BTC (correlated={correlated}/{total}), entry geblokkeerd."
		...
		if segment_threshold and segment_drawdown >= segment_threshold:
			reason = (
				f"Segment {segment} drawdown {segment_drawdown:.2f} EUR >= threshold {segment_threshold:.2f} EUR"
			)
			return RiskDecision(False, reason, metrics)
		...
		if global_block and global_drawdown >= global_block:
			reason = (
				f"Global drawdown {global_drawdown:.2f} EUR >= limit {global_block:.2f} EUR"
			)
			return RiskDecision(False, reason, metrics)
		...
		if max_total > 0:
			...
			if (current_exp + float(amount_eur or 0.0)) > max_total:
				reason = (
					f"Exposure {current_exp + float(amount_eur or 0.0):.2f} EUR zou limiet {max_total:.2f} EUR overschrijden"
				)
				return RiskDecision(False, reason, metrics)
		...
		if profile:
			...
			if max_loss_eur is not None and est_loss is not None and est_loss > max_loss_eur:
				reason = (
					f"Geschatte verlies {est_loss:.2f} EUR overschrijdt stop-profiel limiet {max_loss_eur:.2f} EUR"
				)
				return RiskDecision(False, reason, metrics)
		...
		if daily_limit > 0:
			try:
				...
				if daily_loss >= daily_limit:
					reason = (
						f"🔴 DAILY LOSS LIMIT: €{daily_loss:.2f} losses today "
