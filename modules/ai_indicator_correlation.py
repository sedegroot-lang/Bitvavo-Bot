...
def analyze_coin_specific_patterns(trades: List[dict] = None) -> dict:
    ...
    for market, market_trades in coin_trades.items():
        ...
        losing_scores = [t.get('score', t.get('entry_score', 8)) for t in losses if t.get('score', t.get('entry_score'))]
...
def run_full_correlation_analysis() -> dict:
    ...
    log(f"[INDICATOR] Analysis complete: {len(results.get('coin_patterns', {}).get('coin_patterns', {}))} coins analyzed", level='info')
