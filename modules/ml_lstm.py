...
class LSTMPricePredictor:
    ...
    def __init__(
        self, lookback_window: int = 60, prediction_horizon: int = 5, model_path: str = "models/lstm_price_model.h5"
    ):
        """
        Args:
    ...
    def build_model(self):
        ...
        model = Sequential(
            [
                ...
                LSTM(units=50, return_sequences=True, input_shape=(self.lookback_window, self.features_count)),
                Dropout(0.2),
                # Tweede LSTM laag
    ...
    def normalize_data(self, data: np.ndarray, fit: bool = False) -> np.ndarray:
        ...
        if fit or self.scaler_params is None:
            ...
            self.scaler_params = {"min": data.min(axis=(0, 1)), "max": data.max(axis=(0, 1))}
    ...
    def prepare_sequences(
        ...
        all_data = np.column_stack([price_data, volume_data, rsi_data, macd_data, bb_position_data])
    ...
    def train(
        ...
        history = self.model.fit(
            X_train_norm, y_train, validation_data=validation_data, epochs=epochs, batch_size=batch_size, verbose=1
        )
        ...
        try:
            ...
            if sequence.ndim == 1:
                ...
                if sequence.shape[0] != self.lookback_window or sequence.shape[1] != self.features_count:
                    log(
                        f"LSTM predict: Shape {sequence.shape} != expected ({self.lookback_window}, {self.features_count})",
                        level="debug",
                    )
                    return ("NEUTRAL", 0.33)
                sequence = sequence.reshape(1, self.lookback_window, self.features_count)
        ...
        try:
            ...
            if scaler_path.exists():
                with open(scaler_path, "r") as f:
                    data = json.load(f)
                    self.scaler_params = {"min": np.array(data["min"]), "max": np.array(data["max"])}
...
def create_labels_from_prices(prices: List[float], horizon: int = 5, threshold: float = 0.5) -> np.ndarray:
    """
