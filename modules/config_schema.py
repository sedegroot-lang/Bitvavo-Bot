...
_SCHEMA: Dict[str, Dict[str, Any]] = {
    ...
    "MAX_TOTAL_EXPOSURE_EUR":   {"type": "float", "default": 200.0,"min": 10.0,  "max": 10000.0,"desc": "Max total EUR in trades"},
    ...
    "MIN_SCORE_TO_BUY":         {"type": "float", "default": 7.0,  "min": 0.0,   "max": 15.0,   "desc": "Min signal score to open trade"},
    ...
    "TRAILING_ACTIVATION_PCT":  {"type": "float", "default": 0.045, "min": 0.001, "max": 0.20,   "desc": "Trailing activation threshold"},
    ...
    "MAX_SPREAD_PCT":           {"type": "float", "default": 0.005,"min": 0.001, "max": 0.05,   "desc": "Max spread to enter trade"},
    ...
    "DCA_PYRAMID_MIN_PROFIT_PCT": {"type": "float", "default": 0.03, "min": 0.01, "max": 0.20, "desc": "Min profit % for pyramid"},
    "DCA_PYRAMID_SCALE_DOWN":   {"type": "float", "default": 0.7,  "min": 0.1,   "max": 1.0,   "desc": "Scale factor per pyramid add"},
    "DCA_PYRAMID_MAX_ADDS":     {"type": "int",   "default": 2,    "min": 0,     "max": 10,    "desc": "Max pyramid additions"},  # 0 = disabled
    "DCA_DROP_PCT":             {"type": "float", "default": 0.06, "min": 0.01,  "max": 0.30,   "desc": "Price drop % for DCA trigger"},
    "DCA_AMOUNT_EUR":           {"type": "float", "default": 5.0,  "min": 5.0,   "max": 100.0,  "desc": "EUR per DCA buy"},

    # === Risk management ===
    "RISK_CIRCUIT_BREAKER_EUR": {"type": "float", "default": 50.0, "min": 10.0,  "max": 1000.0, "desc": "Portfolio drawdown circuit breaker EUR"},
    "RISK_DAILY_LOSS_LIMIT_EUR": {"type": "float", "default": 25.0, "min": 5.0,  "max": 500.0,  "desc": "Max daily loss EUR"},
    "RISK_KELLY_ENABLED":       {"type": "bool",  "default": True,                               "desc": "Enable Kelly position sizing"},
    "RISK_KELLY_FRACTION":      {"type": "float", "default": 0.5,  "min": 0.1,   "max": 1.0,   "desc": "Fraction of Kelly to use"},
    ...
    "DUST_TRADE_THRESHOLD_EUR": {"type": "float", "default": 5.0,  "min": 0.1,   "max": 50.0,   "desc": "EUR threshold for dust trade"},
...
def _cross_validate(cfg: dict) -> List[Dict[str, str]]:
    ...
    if isinstance(budget_cfg, dict) and budget_cfg.get("enabled"):
        _g = budget_cfg.get("grid_pct"); grid_pct = float(_g if _g is not None else 20)
        _t = budget_cfg.get("trailing_pct"); trail_pct_b = float(_t if _t is not None else 55)
        _r = budget_cfg.get("reserve_pct"); reserve = float(_r if _r is not None else 25)
    ...
    for key, spec in _SCHEMA.items():
        ...
        if expected_type == "float":
            try:
                ...
                issues.append({"key": key, "issue": f"Expected float, got {type(value).__name__}: {value!r}", "severity": "error"})
                if coerce:
            ...
            try:
                ...
                issues.append({"key": key, "issue": f"Expected int, got {type(value).__name__}: {value!r}", "severity": "error"})
                if coerce:
                    cfg[key] = spec["default"]
                continue
            if "min" in spec and val < spec["min"]:
            ...
            if not isinstance(value, bool):
                ...
                if isinstance(value, str) and value.lower() in ("true", "false", "1", "0", "yes", "no"):
                    ...
                    issues.append({"key": key, "issue": f"Expected bool, got {type(value).__name__}: {value!r}", "severity": "warning"})
