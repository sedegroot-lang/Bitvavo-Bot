...
class AIEngine:
    ...
    @staticmethod
    def macd(vals: List[float], fast: int = 12, slow: int = 26, signal: int = 9) -> Tuple[Optional[float], Optional[float], Optional[float]]:
    ...
    def score_markets(self, markets: Optional[List[str]] = None) -> List[Dict[str, object]]:
        ...
        for m in markets:
            ...
            if self._model is not None:
                ...
                ], key=lambda item: abs(item['impact']), reverse=True)[:3]
    ...
    def recommend_params(self, cfg: Dict, hb: Dict, trades: List[Dict]) -> Dict[str, object]:
        ...
        if regime == 'bull':
            recs.append({'param': 'TRAILING_ACTIVATION_PCT', 'from': trailing_activation, 'to': max(0.01, trailing_activation - 0.003), 'reason': 'bull regime: activate sooner'})
            recs.append({'param': 'RSI_MIN_BUY', 'from': rsi_min_buy, 'to': rsi_min_buy - 1, 'reason': 'bull regime: slightly more aggressive'})
            recs.append({'param': 'BASE_AMOUNT_EUR', 'from': base_amount, 'to': min(100.0, base_amount + 2.0), 'reason': 'bull regime: scale in a bit'})
        elif regime == 'bear':
            recs.append({'param': 'DEFAULT_TRAILING', 'from': default_trailing, 'to': default_trailing + 0.002, 'reason': 'bear regime: tighter trailing'})
            if exposure_ratio >= 0.7:
                recs.append({'param': 'RSI_MIN_BUY', 'from': rsi_min_buy, 'to': rsi_min_buy + 3, 'reason': 'bear regime & high exposure: tighten entries'})
                recs.append({'param': 'BASE_AMOUNT_EUR', 'from': base_amount, 'to': max(5.0, base_amount - 3.0), 'reason': 'bear regime & high exposure: cut size'})
                cooldown = int(cfg.get('OPEN_TRADE_COOLDOWN_SECONDS', 0) or 0)
                target_cooldown = max(cooldown, 900)
                recs.append({'param': 'OPEN_TRADE_COOLDOWN_SECONDS', 'from': cooldown, 'to': min(3600, target_cooldown), 'reason': 'bear regime & high exposure: pause new trades'})
            else:
                recs.append({'param': 'RSI_MIN_BUY', 'from': rsi_min_buy, 'to': rsi_min_buy + 2, 'reason': 'bear regime: avoid weak momentum'})
                recs.append({'param': 'BASE_AMOUNT_EUR', 'from': base_amount, 'to': max(5.0, base_amount - 2.0), 'reason': 'bear regime: reduce risk'})
        else:  # chop
            recs.append({'param': 'TRAILING_ACTIVATION_PCT', 'from': trailing_activation, 'to': trailing_activation, 'reason': 'chop regime: steady'})

        if avg_vol >= 0.03:
            recs.append({'param': 'DCA_SIZE_MULTIPLIER', 'from': dca_mult, 'to': max(0.5, dca_mult - 0.05), 'reason': 'high volatility: reduce DCA size'})
        elif avg_vol <= 0.015:
            recs.append({'param': 'DCA_SIZE_MULTIPLIER', 'from': dca_mult, 'to': min(1.5, dca_mult + 0.05), 'reason': 'low volatility: slightly increase DCA size'})
    ...
    def get_portfolio_status(self) -> Dict[str, object]:
        ...
        try:
            ...
            return {
                ...
                'cash_pct': round((positions[0]['value_eur'] / total_eur * 100) if positions and positions[0]['symbol'] == 'EUR' else 0.0, 2)
            }
        except Exception as e:
            log(f"AIEngine: portfolio status error: {e}", level='error')
            return {'error': str(e)}

    def calculate_optimal_position_size(self, market: str, portfolio: Dict, risk_tolerance: float = 0.05) -> Dict[str, float]:
        """
        ...
        try:
            ...
            if num_crypto >= 10:
                ...
                warnings.append({
                    'type': 'diversification',
                    'severity': 'high',
                    'message': "Geen diversificatie: slechts 1 positie"
                })
                risk_score += 20
            ...
            if cash_pct < 10:
                warnings.append({
                    'type': 'liquidity',
                    'severity': 'high',
                    'message': f"Weinig cash reserve: {cash_pct:.1f}% EUR"
                })
                risk_score += 20
            elif cash_pct > 70:
                warnings.append({
    ...
    def calculate_correlation_matrix(self, markets: Optional[List[str]] = None, lookback: int = 100) -> Dict[str, object]:
        """
    ...
    def calculate_optimal_take_profit(self, market: str, entry_price: float, trade_history: List[Dict]) -> Dict[str, object]:
        """
    ...
    def _tp_recommendation(self, avg_profit: float, volatility: float) -> str:
        ...
        if volatility > 0.04:
            ...
            return f'Normale volatiliteit - standaard targets OK'
        else:
    ...
    def analyze_market_blacklist(self, trade_history: List[Dict]) -> Dict[str, object]:
        ...
        try:
            ...
            for market, stats in market_stats.items():
                ...
                if stats['losses'] > 0:
                    loss_trades = [t for t in trade_history 
                                 if t.get('market') == market and t.get('profit', 0) < 0]
                    if loss_trades:
                ...
                if (stats['trades'] >= 5 and 
                    stats['win_rate'] < 30 and 
                    stats['total_profit'] < -10):
            ...
            return {
                ...
                'recommendation': self._generate_blacklist_recommendation(
                    blacklist_candidates, underperformers
                )
            }
        except Exception as e:
    ...
    def _generate_blacklist_recommendation(self, blacklist: List[Dict], 
                                          underperformers: List[Dict]) -> str:
