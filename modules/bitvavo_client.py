...
def _should_retry(error: Exception) -> bool:
    ...
    retryable_patterns = [
        ...
        'service unavailable', '503', '502', '504', 'reset by peer',
        'temporary', 'retry', 'busy'
    ]
    ...
    for attempt in range(max_retries + 1):
        try:
            ...
            if attempt < max_retries and _should_retry(e):
                ...
                logger.warning(f"Retryable error on attempt {attempt + 1}/{max_retries + 1}: {e}. Retrying in {delay:.1f}s...")
                time.sleep(delay)
            else:
                break
    raise last_error
