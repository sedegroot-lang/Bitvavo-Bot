...
BULLISH_KEYWORDS = [
    'bullish', 'surge', 'rally', 'breakout', 'moon', 'pump', 'soar', 'jump',
    'gain', 'rise', 'bull run', 'all-time high', 'ath', 'adoption', 'partnership',
    'institutional', 'upgrade', 'launch', 'mainnet', 'halving', 'buy signal',
    'accumulation', 'whale buying', 'outperform', 'breakthrough', 'milestone',
    'positive', 'optimistic', 'growth', 'recovery', 'strong', 'momentum'
]

BEARISH_KEYWORDS = [
    'bearish', 'crash', 'dump', 'plunge', 'sell-off', 'selloff', 'drop', 'fall',
    'decline', 'bear market', 'correction', 'fud', 'hack', 'exploit', 'rug pull',
    'scam', 'ban', 'regulation', 'lawsuit', 'sec', 'investigation', 'bankrupt',
    'insolvency', 'liquidation', 'capitulation', 'death cross', 'sell signal',
    'negative', 'pessimistic', 'weak', 'fear', 'panic', 'warning', 'risk'
]
...
def analyze_sentiment_accuracy() -> dict:
    ...
    try:
        ...
        bullish_at_buy = sum(1 for h in history 
                           if h.get('action') == 'BUY' 
                           and h.get('sentiment', {}).get('overall_score', 0) > 0)
...
def get_sentiment_adjustment(coin: str) -> dict:
    ...
    # Extreme greed
        ...
        adjustments['MIN_SCORE_TO_BUY'] = {
            'direction': 'increase', 'magnitude': 0.5 * confidence_mult,
            'reason': f'Fear/Greed {fear_greed} (extreme greed) - be selective'
        }
        adjustments['DEFAULT_TRAILING'] = {
            'direction': 'decrease', 'magnitude': 0.005 * confidence_mult,
            'reason': f'Extreme greed - take profits faster'
        }

    # High BTC volatility — reduce position sizes (real data only)
    if is_real and vol > 2.0:
        adjustments['BASE_AMOUNT_EUR'] = {
            'direction': 'decrease', 'magnitude': 0.85,
            'reason': f'BTC volatility {vol:.1f}x elevated - reduce sizing'
        }
    elif is_real and vol < 0.6 and momentum > 0.01:
        adjustments['BASE_AMOUNT_EUR'] = {
            'direction': 'increase', 'magnitude': 1.1,
            'reason': f'Low volatility + positive momentum - favorable conditions'
        }

    # Weekend adjustments
    if sentiment.get('weekend'):
        adjustments.setdefault('BASE_AMOUNT_EUR', {
            'direction': 'decrease', 'magnitude': 0.85,
