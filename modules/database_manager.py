...
class DatabaseManager:
    ...
    def update_trade(self, market: str, updates: Dict[str, Any]):
        ...
        with self.get_connection() as conn:
            ...
            """,
                values,
            )  # nosec B608 - set_clause built from internal dict keys, not user input
            conn.commit()
    ...
    def get_statistics(self, days: Optional[int] = None) -> Dict[str, Any]:
        ...
        with self.get_connection() as conn:
            ...
            """,
                params,
            ).fetchone()  # nosec B608 - where_clause is internal literal with parameter placeholders
    ...
    def export_to_json(self, output_path: str = "data/trade_log_export.json"):
        ...
        data = {"open": {trade["market"]: trade for trade in open_trades}, "closed": closed_trades}
