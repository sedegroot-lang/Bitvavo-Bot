...
def evaluate_pending_suggestions():
    ...
    for outcome in feedback['suggestion_outcomes']:
        ...
        trades_after = [
            t for t in trades 
            if t.get('close_time', t.get('sell_time', 0)) > applied_at
        ]
        ...
        eff['avg_pnl_impact'] = (
            eff['avg_pnl_impact'] * (eff['total_suggestions'] - 1) + pnl_improvement
        ) / eff['total_suggestions']
        
        log(f"[FEEDBACK] Evaluated {param}: {verdict} (PnL: {pnl_improvement:+.2f}€, WR: {wr_improvement:+.1%})", 
...
def get_ai_performance_summary() -> dict:
    ...
    return {
        ...
        'ai_quality': 'EXCELLENT' if overall_success_rate > 0.7 else (
            'GOOD' if overall_success_rate > 0.55 else (
            'MODERATE' if overall_success_rate > 0.45 else 'NEEDS_IMPROVEMENT'
        )),
        'timestamp': time.time()
    }
...
def run_feedback_cycle():
    ...
    log(f"[FEEDBACK] Cycle complete: {eval_result['evaluated']} evaluated, "
        f"success rate: {summary.get('overall_success_rate', 0):.1%}", level='info')
...
if __name__ == '__main__':
    ...
    for param, data in list(effectiveness.get('parameters', {}).items())[:5]:
        print(f"  {param}: {data['success_rate']:.1%} success, €{data['avg_pnl_impact']:.2f} avg impact - {data['recommendation']}")
