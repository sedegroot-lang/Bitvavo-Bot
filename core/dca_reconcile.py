    ...
    *,
    ...
    if not fills:
        return ReconcileResult(
            market=market, exchange_dca_count=0, bot_dca_count=0,
            events_added=0, events_total=0,
            amount_corrected=False, invested_corrected=False,
            buy_price_corrected=False, repairs=["No fills found on Bitvavo"],
        )
    ...
    if not orders:
        return ReconcileResult(
            ...
            events_added=0, events_total=0,
            amount_corrected=False, invested_corrected=False,
            buy_price_corrected=False, repairs=["No orders after grouping"],
        )
    ...
    if not dry_run:
        ...
        if exchange_total_amount > 0 and bot_amount > 0:
            ...
            if diff_pct > 0.001:  # >0.1% difference
                repairs.append(
                    f"amount: {bot_amount:.8f} → {exchange_total_amount:.8f} "
        ...
        if expected_invested > 0 and bot_invested > 0:
            ...
            if diff_pct > 0.01:  # >1% difference
                repairs.append(
                    f"invested_eur: €{bot_invested:.2f} → €{expected_invested:.2f} "
                    f"(diff {diff_pct*100:.2f}%)"
                )
        ...
        if new_amount > 0 and new_total > 0:
            ...
            if bot_buy_price > 0:
                ...
                if bp_diff > 0.01:  # >1% difference
                    repairs.append(
                        f"buy_price: {bot_buy_price:.6f} → {expected_bp:.6f} "
                        f"(diff {bp_diff*100:.2f}%)"
                    )
    ...
    for market, trade in open_trades.items():
        ...
        try:
            result = reconcile_trade(
                bitvavo, market, trade, dca_max=dca_max, dry_run=dry_run,
            )
            ...
            results.append(ReconcileResult(
                ...
                events_added=0, events_total=0,
                amount_corrected=False, invested_corrected=False,
                buy_price_corrected=False, repairs=[f"Error: {exc}"],
            ))
