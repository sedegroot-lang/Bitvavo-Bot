...
class ReservationManager:
    ...
    def _cleanup_expired(self) -> int:
        ...
        now = time.time()
        expired = [
            market for market, res in self._reservations.items()
        ...
        for market in expired:
            ...
            if res:
                ...
                self._log(
                    f"Reservation expired: {market} (age={res.age:.1f}s, reason={res.reason})",
                    level='warning'
                )
        return len(expired)
    ...
    def active_reservations(self) -> Dict[str, float]:
        ...
        with self._lock:
            self._cleanup_expired()
            return {
                market: res.timestamp 
                for market, res in self._reservations.items()
            }
    ...
    def force_release(self, market: str) -> bool:
        ...
        with self._lock:
            ...
            if res:
                self._log(
                    f"Force released: {market} (was held by thread {res.thread_id})",
                    level='warning'
                )
                return True
            return False
        ...
        with self._lock:
            return {
                ...
                'success_rate': (
                    ...
                    if self._total_reservations > 0 else 100.0
                ),
            }
...
class MarketReservation:
    ...
    def __enter__(self) -> bool:
        self._reserved = self._manager.reserve(
            self.market,
