...
def update_btc_momentum(btc_candles_5m: List) -> Dict[str, Any]:
    ...
    if burst_active:
        ...
        logger.info(f"[BTC-CASCADE] BTC {direction} detected: ROC_5m={roc_1:+.3f}%, "
...
def cascade_score_bonus(
    ...
    if burst_active and burst_dir > 0:
        ...
        if lag > 0.15 and correlation > 0.3:
            ...
            reasons.append(f"btc_pump_lag (expected={expected_alt_move:+.3f}%, actual={actual_alt_move:+.3f}%, lag={lag:.3f}%)")
        elif lag > 0.05:
    ...
    if bonus != 0:
        logger.debug(f"[BTC-CASCADE] {market}: bonus={bonus:+.2f} "
