...
class AutoRetrain:
    ...
    def _has_significant_new_data(self, threshold: int = 50) -> bool:
        ...
        try:
            ...
            new_trades = sum(1 for trade in closed_trades if trade.get("close_timestamp", 0) > last_training_time)
    ...
    def execute_retrain(self) -> Dict:
        ...
        try:
            ...
            log.info("[AUTO_RETRAIN] Training complete!")
            log.info(f"[AUTO_RETRAIN] Accuracy: {metrics['accuracy']:.2%}")
            log.info(f"[AUTO_RETRAIN] ROC-AUC: {metrics['roc_auc']:.2%}")

            return {"success": True, "metrics": metrics, "samples": len(df), "timestamp": timestamp}
    ...
    def run_check(self) -> Dict:
        ...
        if should_train:
            ...
            return {"action": "skipped", "reason": reason, "timestamp": datetime.now().isoformat()}
