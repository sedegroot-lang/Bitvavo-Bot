...
def train_enhanced_model(X: np.ndarray, y: np.ndarray) -> Tuple[xgb.XGBClassifier, Dict]:
    ...
    metrics = {
        ...
        "feature_importance": dict(
            zip([f"feature_{i}" for i in range(X.shape[1])], model.feature_importances_.tolist())
        ),
...
def main():
    ...
    # Quality check
    meets_threshold = metrics["test_auc"] >= TARGET_AUC and metrics["test_precision_1"] >= TARGET_PRECISION_CLASS1
