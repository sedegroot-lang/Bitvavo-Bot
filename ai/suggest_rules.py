...
def rules_basic_performance(ctx: RuleCtx) -> List[Suggestion]:
    ...
    if pnl < 0 and open_exposure > 0.5 * max_total:
        s = _suggest(
            "DEFAULT_TRAILING",
            float(cfg.get("DEFAULT_TRAILING", 0.012)),
            float(cfg.get("DEFAULT_TRAILING", 0.012)) + 0.002,
            "loss+high exposure",
        )
        if s:
            out.append(s)
    ...
    if len(pnl_list) >= 10:
        ...
        if len(recent_losses) >= 5:
            ...
            s = _suggest("HARD_SL_ALT_PCT", current, max(0.025, current - 0.005), "high loss rate: tighten stop-loss")
            if s:
                out.append(s)
        ...
        try:
            ...
            if target is not None:
                s = _suggest(
                    "DCA_AMOUNT_EUR", current_dca, float(target), "adjust DCA size based on recent pnl/exposure"
                )
                if s:
                    out.append(s)
...
def rules_signal_optimization(ctx: RuleCtx) -> List[Suggestion]:
    ...
    if len(pnl_list) >= 10:
        ...
        if profit_factor < 0.5 and len(pnl_list) >= 15:
            ...
            current = float(cfg.get("MIN_SCORE_TO_BUY", 2.0))
            s = _suggest(
                "MIN_SCORE_TO_BUY", current, min(3.0, current + 0.25), f"Low WR ({win_rate:.1%}): slightly stricter"
            )
            if s:
                out.append(s)
    ...
    if win_rate < 0.45 and len(pnl_list) >= 10:
        ...
        s = _suggest_int("RSI_MAX_BUY", current, max(45, current - 5), "low win rate: avoid overbought entries")
        if s:
            out.append(s)
...
def rules_profit_factor(ctx: RuleCtx) -> List[Suggestion]:
    ...
    if len(pnl_list) >= 15:
        ...
        if pf < 0.5:  # CRITICAL
            s = _suggest(
                "DEFAULT_TRAILING",
                ct,
                min(0.09, max(0.07, ct + 0.02)),
                f"ðŸš¨ CRITICAL PF={pf:.2f}: need MUCH bigger wins",
                check_duplicate=False,
                extras={"impact": "CRITICAL", "profit_factor": pf},
            )
            if s:
                out.append(s)
            s = _suggest(
            ...
            s = _suggest("DEFAULT_TRAILING", ct, min(0.05, ct + 0.007), f"Weak PF={pf:.2f}: slightly larger trailing")
            if s:
                out.append(s)
            ...
            if cms > 8:
                s = _suggest(
                    "MIN_SCORE_TO_BUY", cms, max(7.5, cms - 0.5), f"âœ… Excellent PF={pf:.2f}: can lower entry barrier"
                )
                if s:
                    out.append(s)
        elif pf > 2.0 and avg_win > avg_loss * 1.8:  # OUTSTANDING
            s = _suggest(
                "DEFAULT_TRAILING", ct, max(0.025, ct - 0.005), f"ðŸ† Outstanding PF={pf:.2f}: take profits sooner"
            )
            if s:
                out.append(s)
            if cba < 25:
    ...
    if len(pnl_list) >= 10:
        ...
        if dca > base * 3:
            ...
            s = _suggest(
                "DCA_AMOUNT_EUR", dca, target, f"DCA â‚¬{dca} too large vs BASE â‚¬{base}: aligning to {target:.1f}"
            )
            if s:
                out.append(s)
        elif win_rate > 0.60 and dca < base * 1.5:
            target = min(base * 2, dca * 1.3)
            s = _suggest("DCA_AMOUNT_EUR", dca, target, f"win rate {win_rate:.1%}: can increase DCA size")
            if s:
                out.append(s)
...
def rules_advanced_optimization(ctx: RuleCtx) -> List[Suggestion]:
    ...
    if len(pnl_list) >= _GATE_MED:
        ...
        if len(trend_trades) >= 8:
            ...
            if trend_wr < 0.45 and sma_short > 5:
                s = _suggest_int(
                    "SMA_SHORT", sma_short, max(5, sma_short - 2), f"trend WR {trend_wr:.1%}: try faster SMA"
                )
                if s:
                    out.append(s)
    ...
    if len(pnl_list) >= _GATE_MED:
        ...
        if pnl_std > 2.0 and not vol_enabled and _cooldown_ok("VOLATILITY_SIZING_ENABLED"):
            out.append(
                {
                    "param": "VOLATILITY_SIZING_ENABLED",
                    "from": False,
                    "to": True,
    ...
    if len(pnl_list) >= _GATE_LOW:
        ...
        low_vol_trades = [
            t for t in closed_trades[-20:] if t.get("volume_24h_eur", float("inf")) < current_min_vol * 1.5
        ]
        if len(low_vol_trades) >= 5:
            ...
            if low_vol_wr < 0.35:
                s = _suggest(
                    "MIN_VOLUME_24H_EUR",
                    current_min_vol,
                    min(180000, current_min_vol + 25000),
...
def rules_market_intelligence(ctx: RuleCtx) -> List[Suggestion]:
    ...
    if len(pnl_list) >= _GATE_MED:
        ...
        if qualified:
            sorted_coins = sorted(
                qualified.items(), key=lambda x: x[1]["win_rate"] * max(x[1]["avg_pnl"], 0.1), reverse=True
            )
            for coin, stats in sorted_coins[:2]:
                if coin not in (cfg.get("WHITELIST_MARKETS") or []):
                    log(
                        f"[COIN-LEARNING] {coin} performs well: WR={stats['win_rate']:.1%}, "
                        f"avg PnL=â‚¬{stats['avg_pnl']:.2f}",
                        level="info",
                    )
            for coin, stats in sorted_coins[-3:]:
                if coin in (cfg.get("WHITELIST_MARKETS") or []):
                    if stats["win_rate"] < 0.35 or stats["avg_pnl"] < -2:
                        log(
                            f"[COIN-LEARNING] {coin} performs poorly: WR={stats['win_rate']:.1%}, "
                            f"avg PnL=â‚¬{stats['avg_pnl']:.2f}",
                            level="warning",
                        )
        ...
        try:
            ...
            try:
                with open(trade_log_path, "r") as f:
                    ...
                    if isinstance(trade_data, dict):
                        open_trades_data = {
                            t["market"]: t for t in trade_data.get("open", []) if t.get("status") == "open"
                        }
            except Exception as e:
            ...
            if open_trades_data:
                ...
                if div_score < 0.5 and total >= 3:
                    log(
                        f"[CORRELATION] ðŸ“Š Low diversification: {unique_sectors} sectors across {total} trades",
                        level="info",
                    )
        except Exception as e:
    ...
    if btc_trend and btc_trend.get("trend_score") is not None:
        ...
        if btc_volatility < 1.0 and btc_momentum > 0.01:
            ...
            s = _suggest(
                "BASE_AMOUNT_EUR",
                current,
                min(100.0, current * 1.1),
                "BTC calm uptrend: increase size",
                extras={"source": "btc_candles", "data_quality": "real"},
            )
            if s:
                s["priority"] = 75
        ...
        if score_stats:
            if regime == "BULL" and regime_data.get("confidence", 0) > 0.6:
                ...
                if candidates:
                    ...
                    if best < current_min_score and _cooldown_ok("MIN_SCORE_TO_BUY"):
                        out.append(
                            {
                                "param": "MIN_SCORE_TO_BUY",
                                "from": current_min_score,
                                "to": best,
            ...
            if score_stats:
                ...
                for sc in sorted(score_stats.keys(), reverse=True):
                    d = score_stats[sc]
                    log(
                        f"  Score {sc}: {d['win_rate'] * 100:.1f}% WR, â‚¬{d['avg_pnl']:.2f} avg, {d['trades']} trades",
                        level="info",
                    )
    ...
    if len(pnl_list) >= _GATE_LOW:
        ...
        if total_pnl > 50 and win_rate > 0.55 and current_portion < 0.7:
            ...
            if _cooldown_ok("REINVEST_PORTION"):
                out.append(
                    {
                        "param": "REINVEST_PORTION",
                        "from": current_portion,
                        "to": new_portion,
...
def rules_trailing_entry_tp(ctx: RuleCtx) -> List[Suggestion]:
    ...
    if len(pnl_list) >= _GATE_LOW and cfg.get("TAKE_PROFIT_ENABLED", True):
        ...
        if len(winners) >= 5:
            ...
            if avg_win < 0.5 and pct1 > 0.25:
                s = _suggest(
                    "PARTIAL_TP_SELL_PCT_1", pct1, max(0.2, pct1 - 0.05), f"Avg win {avg_win:.2f}: sell less at TP1"
                )
                if s:
                    out.append(s)
            elif avg_win > 2.0:
    ...
    if len(pnl_list) >= _GATE_LOW and cfg.get("TRAILING_ENTRY_ENABLED", False):
        ...
        if len(te_trades) >= 5 and len(nm_trades) >= 5:
            ...
            if te_wr > nm_wr + 0.10 and pullback < 0.02:
                s = _suggest(
                    "TRAILING_ENTRY_PULLBACK_PCT",
                    pullback,
                    min(0.025, pullback + 0.005),
                ...
                if timeout > 60:
                    s = _suggest_int(
                        "TRAILING_ENTRY_TIMEOUT_S", timeout, max(30, timeout - 30), "TE underperforms: shorter timeout"
                    )
                    if s:
                        out.append(s)
    ...
    if len(pnl_list) >= _GATE_LOW:
        ...
        if vol > 1.5 and max_iter > 1:
            s = _suggest_int(
                "DCA_MAX_BUYS_PER_ITERATION", max_iter, 1, f"High volatility ({vol:.1f}x): limit DCA per iteration"
            )
            if s:
                out.append(s)
        elif vol < 0.8 and win_rate > 0.55 and max_iter < 3:
            s = _suggest_int("DCA_MAX_BUYS_PER_ITERATION", max_iter, max_iter + 1, "Low vol + good WR: DCA faster")
            if s:
                out.append(s)
...
def coherence_check(out: List[Suggestion], cfg: dict) -> List[Suggestion]:
    ...
    if "BASE_AMOUNT_EUR" in pending and "DCA_AMOUNT_EUR" not in pending:
        ...
        if cur_dca > new_base * 1.5:
            ...
            s = _suggest(
                "DCA_AMOUNT_EUR", cur_dca, tgt_dca, f"Coherence: base->{new_base:.0f}, DCA align->{tgt_dca:.0f}"
            )
            if s:
                extra.append(s)
    ...
    if tp1 >= tp2:
        s = _suggest("TAKE_PROFIT_TARGET_2", tp2, tp1 + 0.015, f"Coherence: TP2 ({tp2:.3f}) must be > TP1 ({tp1:.3f})")
        if s:
            extra.append(s)
    if tp2 >= tp3:
        s = _suggest("TAKE_PROFIT_TARGET_3", tp3, tp2 + 0.02, f"Coherence: TP3 ({tp3:.3f}) must be > TP2 ({tp2:.3f})")
        if s:
            extra.append(s)
    ...
    if "HARD_SL_ALT_PCT" in pending and "DEFAULT_TRAILING" not in pending:
        ...
        if new_sl < cur_trail * 2:
            ...
            if cur_trail > ideal + 0.005:
                s = _suggest(
                    "DEFAULT_TRAILING", cur_trail, ideal, f"Coherence: SL->{new_sl:.3f}, trail tighten->{ideal:.3f}"
                )
                if s:
                    extra.append(s)
            ...
            if queue_market_for_watchlist:
                for rec in high_prio[:2]:
                    ...
                    if market not in current_wl:
                        if queue_market_for_watchlist(
                            market, reason=rec.get("reason", "ai-scan"), source="ai-supervisor"
                        ):
